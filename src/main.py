...
import datetime #tiempo de expiracion del token
...
def register():
    if request.method == 'POST':
        ...
        user = User()
        user.email = email
        #Crea un codigo con la funcion hash y se lo asigno a password
...
def login():
    if request.method == 'POST':
        ...
        user = User.query.filter_by(email=email).first()
        if not user:
            return jsonify({"Mensaje": "El user es incorrecto"}), 401
        #cheque el password y lo incripta
